...
// Cada version de la app renueva la caché (medicion-obra-v55<N>); el CI lo auto-incrementa.
const CACHE = 'medicion-obra-v55';
const ASSETS = [
  './',
  './mediotec.html',
  './manifest.json',
  ...
  e.waitUntil(
    caches.keys().then(keys => {
      ...
      return Promise.all(oldKeys.map(k => caches.delete(k))).then(() => {
        ...
        if (oldKeys.length > 0) {
          const ver = CACHE.replace('medicion-obra-v55','');
